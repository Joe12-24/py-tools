                    ...
                    if match:
                        ...
                        if match:
                            ...
                            for element2 in element.iter():
                                if element2.tag == 'foreach':
                                    foreach_found = True
                                    break

                            # 如果含有foreach标签，则打印update标签的id
                            if foreach_found:
                                print("foreach:"+file_path+"\t"+tag_id+"\t"+table_name)
                                # if 'foreach'  in tag_text:
                                #     # 将修改后的内容写回文件
                            ...
                            if "LCODER_UPDATE_TIME = sysdate" not in tag_text:
                                # 将修改后的内容写回文件
                                with open(file_path, 'r' ,encoding='utf-8') as file:
                                    data = file.read()
